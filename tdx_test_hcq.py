...
def parse_gpcw_file(gpcw_path, target_code):
    ...
    # 计算复权因子（后复权）
    df['factor'] = 1.0
    for i in range(len(df)-1, -1, -1):  # 从后向前计算
...
# 2. 应用后复权调整
# ----------------------
def apply_backward_adjustment(stock_data, df_gpcw):
    """
    应用后复权调整到日线数据
    """
    # 合并权息日期和复权因子
...
if __name__ == "__main__":
    ...
    # 应用后复权
    adjusted_data = apply_backward_adjustment(stock_data, df_gpcw)

    # 合并原始数据和复权数据
    result = pd.concat([stock_data, adjusted_data], axis=1)

    # 保存结果
    result.to_csv(f'sh{target_code}_adjusted.csv')

    # 可视化对比
    plt.figure(figsize=(12, 6))
    plt.plot(result['close'], label='原始价格', alpha=0.5)
    plt.plot(result['close_adj'], label='后复权价格', linestyle='--')
    plt.title(f'股票 {target_code} 后复权价格对比')
    plt.legend()
    plt.show()
